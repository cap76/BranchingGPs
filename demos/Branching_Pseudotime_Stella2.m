function Output = Branching_Pseudotime_Stella2(seed,missingT);
...
try
    dasdasda load(['~/Desktop/BranchingGPs/results/primordial_germ_cells/Pseudotime/Marker_Pseudotime_' num2str(seed) '_' num2str(missingT) '_2E.mat'])
    ...
    if double(int64(i/100))==(i/100)
     ...
     %Every 100 steps do HMC on hyperparams. Maybe break this down ... the branch times are what we want to update really. 
         ...
         if double(int64(i/10))==(i/10)
            ...
            save(['~/Desktop/BranchingGPs/results/primordial_germ_cells/Pseudotime/Stella_Pseudotime_' num2str(seed) '_' num2str(missingT) '_2E_final.mat'],'Output')            
...
save(['~/Desktop/BranchingGPs/results/primordial_germ_cells/Pseudotime/Marker_Pseudotime_' num2str(seed) '_' num2str(missingT) '_2E_final_rerun.mat'],'Output')            
...
D1 = importdata('/Users/christopher_penfold/Desktop/Yun Revision/Figure_1.xlsx')
...
for i = 1:size(D1.data,1)-2    
    [A P(i,1)] = ttest2(D1.data(i+2,inds0),D1.data(i+2,inds1)); %Dynamic
    [A P(i,2)] = ttest2(D1.data(i+2,inds1),D1.data(i+2,inds2)); %Dynamic 
    [A P(i,3)] = ttest2(D1.data(i+2,inds0),D1.data(i+2,inds3)); %Early DE    
    [A P(i,4)] = ttest2(D1.data(i+2,inds1),D1.data(i+2,inds4)); %Late DE        
    [A P(i,5)] = ttest2(D1.data(i+2,inds2),D1.data(i+2,inds5)); %Late DE            
end

ind_dynam = find( (P(:,1)<0.05 & P(:,2)<0.05) & P(:,3)>0.1 & P(:,4)>0.1 & P(:,5)>0.1);

ind_early = find( (P(:,1)<0.05 & P(:,2)<0.05) & P(:,3)<0.05 );
ind_med = find( (P(:,1)<0.05 & P(:,2)<0.05) & P(:,4)<0.05 );
ind_late = find( (P(:,1)<0.05 & P(:,2)<0.05) & P(:,5)<0.05 );

ind_dynam = ind_dynam(randperm(length(ind_dynam)));
ind_early = ind_early(randperm(length(ind_early)));
ind_med = ind_med(randperm(length(ind_med)));
ind_late  = ind_late(randperm(length(ind_late)));

ind = unique([ind_dynam(1:20);ind_early(1:10);ind_med(1:10);ind_late(1:10)]);

%Now get selection of genes
...
Type = [ones(1,33),2*ones(1,33)];            %Cell type
Sex  = [ones(1,33),ones(1,33)];            %Sex
uID  = 1:1:length(t1);          %Unique ID (for rejumbling everything later on)

ind_missing = find(t1==-1); %Get indices for the ESCs/unlabelled. These will be assigned on the fly ...
ind_obs     = setdiff(1:1:length(t1),ind_missing); %Index for everything else
