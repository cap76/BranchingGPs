...
if nargin<4                                                        % covariances  
    ...
    K    = [K2(ind1,ind1)+K1(ind1,ind1),K1(ind1,ind2);K1(ind2,ind1),K1(ind2,ind2)+K3(ind2,ind2)];           
        ...
        K1    = covSEiso([hyp(8),hyp(9)],x(:,1),z(:,1));
        covN1 = {@covChangePointMultiD, {1, @covSEiso, @covZero}};
        %covN2 = {@covChangePointMultiD, {1, @covZero, @covSEiso}};
        %K = zeros(size(x,1),1);
        K2   = feval(covN1{:}, [location,steepness1,hyp(4),hyp(5)], x(:,1),z(:,1));
        K3   = feval(covN1{:}, [location,steepness2,hyp(6),hyp(7)], x(:,1),z(:,1));
        
        %K    = [K2(ind1,ind1)+K1(ind1,ind1),K1(ind1,ind2);K1(ind2,ind1),K1(ind2,ind2)+K3(ind2,ind2)];           
        K = [K1(ind1,ind3)+K2(ind1,ind3),K1(ind1,ind4); K1(ind2,ind3),K1(ind2,ind4)+K3(ind2,ind4)];        
    end
